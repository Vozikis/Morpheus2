...
echo "  EPOCHS=$EPOCHS BATCH_SIZE=$BATCH_SIZE MIN_BATCH_SIZE=$MIN_BATCH_SIZE OOM_RETRIES=$OOM_RETRIES LR=$LR SIGMA_REG_WEIGHT=$SIGMA_REG_WEIGHT"
echo "  D_MODEL=$D_MODEL NHEAD=$NHEAD NUM_LAYERS=$NUM_LAYERS DROPOUT=$DROPOUT CONTEXT_WINDOW=$CONTEXT_WINDOW LOG_SIGMA_MIN=$LOG_SIGMA_MIN LOG_SIGMA_MAX=$LOG_SIGMA_MAX SURPRISE_MODE=$SURPRISE_MODE AMP=$AMP MULTI_GPU=$MULTI_GPU GPU_COUNT=$GPU_COUNT GPU_COUNT_RAW=$GPU_COUNT_RAW DDP_BACKEND=$DDP_BACKEND MASTER_PORT=$MASTER_PORT"
